        ...
        if !ShareManager.thirdAccout(type: .weibo).isAppInstalled {
            MonkeyKing.oauth(for: .weibo) { (info, _, error) in
                if let accessToken = info?["access_token"] as? String {
                    ...
                    self.shareToWechat(URLString: URLString, image: image, description: description, title: title, complete: complete)
                } else {
                    complete(false)
                }
            ...
            shareToWechat(URLString: URLString, image: image, description: description, title: title, complete: complete)
        }
    }
