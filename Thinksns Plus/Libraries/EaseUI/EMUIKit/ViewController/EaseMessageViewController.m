...
//    NSString *replace = [text stringByReplacingOccurrencesOfString:@" " withString:@""];
    ...
    EMMessage *message = [EaseSDKHelper getTextMessage:replace to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:ext];
...
- (void)sendLocationV2Latitude:(double)latitude longitude:(double)longitude address:(NSString *)address title:(NSString *)title image:(UIImage *)image {
    ...
    EMMessage *message = [EaseSDKHelper getImageMessageWithImage:image to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:locationInfo];
...
- (void)sendLocationV2Latitude:(double)latitude longitude:(double)longitude address:(NSString *)address title:(NSString *)title image:(UIImage *)image uid:(NSString *)uid {
    ...
    NSDictionary *locationInfo = @{@"image": @"1", @"address": address, @"title": title, @"latitude": [NSString stringWithFormat:@"%f",latitude], @"longitude": [NSString stringWithFormat:@"%f",longitude]};
    EMMessage *message = [EaseSDKHelper getImageMessageWithImage:image to:uid messageType:[self _messageTypeFromConversationType] messageExt:locationInfo];
    [self _sendMessage:message isNeedUploadFile:YES];
}
...
- (void)sendImageMessageWithData:(NSData *)imageData mimeType: (NSString *)mimeType
{
    ...
    EMMessage *message = [EaseSDKHelper getImageMessageWithImageData:imageData to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:@{@"mimeType": @"image/gif"}];
    [self _sendMessage:message isNeedUploadFile:YES];
}
