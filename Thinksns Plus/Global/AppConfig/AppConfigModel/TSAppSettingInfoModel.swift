...
class TSAppSettingInfoModel: Mappable {
    ...
    /// 匿名规则,这个属性是从question-configs接口中获取的
    ...
    func mapping(map: Map) {
        ...
        walletRechargeType <- map["wallet:recharge-type"]
        ads <- map["ad"]
        quoraApplyAmount <- map["question:apply_amount"]
        quoraOutLookAmount <- map["question:onlookers_amount"]
        // 这个字段是本地拼接的
        quoraAnonymityRule <- map["question:anonymity_rule"]
        newsContributePay <- map["news:contribute.pay"]
        newsContributeVerified <- map["news:contribute.verified"]
        newsContributeAmount <- map["news:pay_conyribute"]
        checkin <- map["checkin"]
        checkBalance <- map["checkin:attach_balance"]
        ...
        showCash <- map["wallet:cash.open"]
        showRecharge <- map["wallet:recharge.open"]
        cashTransform <- map["wallet:transform.open"]
        showIntegration <- map["currency:cash.open"]
        showIntegrationRecharge <- map["currency:recharge.open"]
        showOnlyIAP <- map["currency:recharge.IAP_only"]
