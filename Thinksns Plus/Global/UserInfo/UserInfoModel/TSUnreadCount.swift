...
class TSUnreadCount: NSObject {
    ...
    func allNoticeUnreadCount() -> Int {
        return follows + comments + like + pending + system + at + mutual + newsCommentPinned + feedCommentPinned + groupJoinPinned + postCommentPinned + postPinned
    }

    // 聊天信息\新增好友 不包括在内
    func onlyNoticeUnreadCount() -> Int {
        return comments + like + pending + system + at + newsCommentPinned + feedCommentPinned + groupJoinPinned + postCommentPinned + postPinned
    }
}
