...
class TSKeyboardToolbar: UIView, TSTextToolBarViewDelegate {
    ...
    // 工具栏初始高度
    private let toolBarHeight: CGFloat = 42.0
    // 键盘通知单例
    ...
    private func changeUI(keyboard: Typist.KeyboardOptions, event: Typist.KeyboardEvent) {
        ...
        switch event {
        case .willShow:
            setKeyboardToolbarOrigin(keyboard: keyboard, keyboardToolbar: keyboardToolbar, mWindow: mWindow)
    ...
    private func setKeyboardToolbarOrigin(keyboard: Typist.KeyboardOptions, keyboardToolbar: TSKeyboardToolbar, mWindow: UIWindow) {
        originY = keyboard.endFrame.origin.y - toolBarHeight
        keyboardToolbar.frame = CGRect(x: 0, y: keyboard.endFrame.origin.y - (toolBarHeight + changeHeight), width: mWindow.bounds.size.width, height: toolBarHeight + changeHeight)
    }
