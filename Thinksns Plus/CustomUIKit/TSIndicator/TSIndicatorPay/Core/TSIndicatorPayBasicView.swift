...
class TSIndicatorPayBasicView: UIView, TYAttributedLabelDelegate {
    ...
    func setUI() {
        ...
        // 灰色分割线
        lineGray.frame = CGRect(x: (whiteViewWidth - buttonWidth) / 2, y: 54, width: buttonWidth, height: 1)
        lineGray.backgroundColor = TSColor.inconspicuous.highlight
        // 标题
...
//        addGestureRecognizer(tap)
    ...
    public func setDescription(content: String?, linkWord: String?) {
        labelForDescription.text = content
        if let linkWord = linkWord {
            labelForDescription.text = content == nil ? "" : content! + linkWord
            let location = content == nil ? 0 : content!.count
