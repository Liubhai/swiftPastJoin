...
class TSUserInterfacePrinciples: NSObject {
    ...
    func isiphoneX() -> Bool {
        ...
        if identifier == "i386" || identifier == "x86_64" {
            // 模拟器根据屏幕尺寸判断
            // CGSizeMake(375, 812)
            if UIScreen.main.bounds.size.equalTo(CGSize(width: 375, height: 812)) == true || UIScreen.main.bounds.size.equalTo(CGSize(width: 812, height: 375)) {
                return true
            } else {
                return false
            }
        } else if identifier == "iPhone10,3" || identifier == "iPhone10,6" {
            return true
        } else {
            return false
        }
