...
extension WalletNetworkManager {
    ...
    ///   - complete: 结果
    class func createRecharge(type: String, amount: Int, extra: Any?, complete: @escaping (Bool, String?, WalletCreateCharegeModel?) -> Void) {
        // 1.请求 url
        var request = WalletNetworkRequest.Recharge.create
        request.urlPath = request.fullPathWith(replacers: [])
        // 2.配置参数
        let parameters: [String: Any] = ["amount": amount, "type": type]
        request.parameter = parameters
        // 3.发起请求
        RequestNetworkData.share.text(request: request) { (networkResult) in
            switch networkResult {
