...
class ReceiveLikeTableVC: TSTableViewController, NoticePendingProtocol {
    ...
    override func refresh() {
        ...
        request.parameter = ["type": "liked"]
        RequestNetworkData.share.text(request: request) { (_) in
            // 直接清理本地的数据
            TSCurrentUserInfo.share.unreadCount.like = 0
        }
        NoticeReceiveInfoNetworkManager.receiveLikeList(after: nil) { [weak self] (models, _) in
    ...
    override func loadMore() {
        ...
        let after = model.id
        NoticeReceiveInfoNetworkManager.receiveLikeList(after: after) { [weak self] models, _ in
            guard let weakSelf = self else {
                return
            }
