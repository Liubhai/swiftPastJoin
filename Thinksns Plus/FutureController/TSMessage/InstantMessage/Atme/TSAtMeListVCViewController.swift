...
class TSAtMeListVCViewController: TSTableViewController, NoticePendingProtocol, TSKeyboardToolbarDelegate {
    ...
    override func refresh() {
        NoticeReceiveInfoNetworkManager.receiveAtMeList(index: nil) {[weak self] (models, msg) in
    ...
    override func loadMore() {
        ...
        NoticeReceiveInfoNetworkManager.receiveAtMeList(index: sourceData.last?.id) {[weak self] (models, msg) in
            guard let weakSelf = self else {
                return
            }
