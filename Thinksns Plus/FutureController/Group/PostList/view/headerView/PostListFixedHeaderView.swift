...
class PostListFixedHeaderView: UIView {
    ...
    func loadMemberCountLabel() {
        ...
        memberLabel.text = "成员 \(model.memberCount)"
    ...
    func loadIntroLabel(yRecord: inout CGFloat) {
        ...
        addUnfoldButton()
        let strings = [model.intro]
        let colors = [UIColor(hex: 0x333333)]
        introLabel.attributedText = NSMutableAttributedString.attributeStringWith(strings: strings, colors: colors, fonts: [14])
        introLabel.sizeToFit()
        introLabel.frame = CGRect(origin: CGPoint(x: 50, y: seperatorLine.frame.maxY + 12), size: introLabel.size)
    ...
    func addUnfoldButton() {
        ...
        // 2.2 配置按钮标题
        let foldTitle = QuoraStackBottomButtonsCell.getAttributeString(texts: ["...", "查看更多"], colors: [TSColor.normal.content, TSColor.main.theme])
    ...
    func addClosseBtn() {
        ...
        highlight.tapAction = {[weak self] (containerView, text, range, rect) in
            ...
            weakself.introLabel.sizeToFit()
            weakself.introLabel.frame = CGRect(origin: CGPoint(x: 50, y: (weakself.seperatorLine.frame.maxY + 12)), size: weakself.introLabel.size)
            // 更新 yRecord
        ...
        let foldTitle = QuoraStackBottomButtonsCell.getAttributeString(texts: [self.model.intro, "收起"], colors: [TSColor.normal.content, TSColor.main.theme])
