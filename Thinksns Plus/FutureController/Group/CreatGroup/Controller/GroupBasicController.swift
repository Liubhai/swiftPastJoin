...
class GroupBasicController: UITableViewController, UITextFieldDelegate {
    ...
    let rightButton: UIButton = {
        ...
        button.setTitleColor(UIColor(hex: 0x59b6d7), for: .normal)
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if textField == nameTextField {
            return TSCommonTool.stringContainsEmoji(string) ? false : true
        }
        return true
    }
    /// ”同步至动态“的开关被点击了
...
//        cameraVC.show()
    ...
    private func openLibrary() {
        ...
        guard let imagePickerVC = TZImagePickerController(maxImagesCountTSType: 1, columnNumber: 4, delegate: self, pushPhotoPickerVc: true, square: true, shouldPick: true, topTitle: "更换头像")
            else {
                return
        }
...
//        albumVC.show()
    ...
    func imagePickerController(_ picker: TZImagePickerController!, didFinishPickingPhotos photos: [UIImage]!, sourceAssets assets: [Any]!, isSelectOriginalPhoto: Bool) {
        if photos.count > 0 {
            ...
            self.model.coverImage = photos[0]
            self.userOperated()
        } else {
