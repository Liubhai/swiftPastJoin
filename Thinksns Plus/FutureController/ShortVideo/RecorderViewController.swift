...
class RecorderConsole: UIView {
    ...
    /// 录制器进度条
    private let recorderProgressView = RecorderProgressView(frame: CGRect(x: 0, y: TSLiuhaiHeight, width: UIScreen.main.bounds.width, height: 4), minDuration: 3, maxDuration: 10)
    /// 录制按钮
...
class RecorderViewController: UIViewController {
    ...
    /// 控制台
    let recorderConsole: RecorderConsole = RecorderConsole(frame: UIScreen.main.bounds)
    var focusView: SCRecorderToolsView = SCRecorderToolsView()
    var recorder: SCRecorder!
    weak var delegate: RecorderVCDelegate?
    fileprivate let maxDuration: Int = 10
