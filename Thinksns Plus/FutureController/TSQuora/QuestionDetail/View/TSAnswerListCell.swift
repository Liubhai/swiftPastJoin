...
//    fileprivate weak var shortContentLabel: UILabel!
    ...
    func loadAnswer(_ answer: TSAnswerListModel, questionUserId: Int?, showTag: Bool = true) -> Void {
        self.model = answer
        self.setupWithModel(answer, questionUserId: questionUserId, showTag: showTag)
        self.toolBarEnable = true
    }
    ...
    private func initialTopView(_ topView: UIView) -> Void {
        ...
        // 4.1 invitationView
        let invitationView = UIView(cornerRadius: 2, borderWidth: 1, borderColor: UIColor(hex: 0x59b6d7))
        ...
        // 4.2 invitationLabel
        let invitationLabel = UILabel(text: "", font: UIFont.systemFont(ofSize: 10), textColor: UIColor(hex: 0x59b6d7))
    ...
    // MARK: - Private  数据加载
    /// 数据加载
    private func setupWithModel(_ model: TSAnswerListModel, questionUserId: Int?, showTag: Bool = true) -> Void {
        // 用户信息， 注意匿名展示 - 可优化
