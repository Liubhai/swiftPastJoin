...
class TSCommetDetailTableView: TSMomentDetailVC, UITableViewDataSource, TSDetailCommentTableViewCellDelegate {
    ...
    init(feedId: Int, isTapMore: Bool = false) {
        ...
        requestModel(feedId: feedId)
    }

    /// 请求基本动态数据
    func requestModel(feedId: Int) {
        self.requestMomentData(feedId: feedId, complete: {(cellModel, resposeInfo, code) in
...
//                        self.loadFaild(type: .blank)
    ...
    func showPayAlert(paidInfo: PaidInfo) {
        PaidManager.showFeedPaidTextAlertCallBack(feedId: self.requestId, paidInfo: paidInfo, complete: { (payStatus, content) in
            if payStatus == 0 {
                ...
                /// 刷新该页面数据
                self.loading()
                self.requestModel(feedId: self.requestId)
            } else {
                // 取消或者其他异常
                self.endLoading()
                self.payInfo = nil
    ...
    func showDBData() {
        commentDatas = TSDataQueueManager.share.moment.makeCommentModels(mommentObject: model!.data!, feedId: (model!.data?.feedIdentity)!, isNetWork: false)
        if !commentDatas.isEmpty {
            /// 获取数据库的数据，如果超过20条就截止到20条
            if commentDatas.count <= maxLoadDB {
                super.model!.comments = commentDatas
                // 加载一页数据
                self.refresh()
            } else {
                super.model!.comments = Array(commentDatas[0..<maxLoadDB])
                // 加载一页数据
                self.refresh()
            }
        } else {
    ...
    func refresh() {
        ...
        guard  let feedId = model?.data?.feedIdentity else {
            return
        }
//        requestMomentData(feedId: feedId) {(_, _) in
//        }
    }
