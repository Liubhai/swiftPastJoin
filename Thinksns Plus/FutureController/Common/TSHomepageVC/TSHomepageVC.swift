...
class TSHomepageVC: UIViewController, TZImagePickerControllerDelegate {
...
//            imagePicker.show()
    ...
    fileprivate func openLibrary() {
        guard let imagePickerVC = TZImagePickerController(maxImagesCountTSType: 1, columnNumber: 4, delegate: self, pushPhotoPickerVc: true, square: false, shouldPick: true, topTitle: "更换封面")
            else {
                return
        }
