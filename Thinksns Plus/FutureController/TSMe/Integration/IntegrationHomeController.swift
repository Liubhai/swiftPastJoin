        ...
        IntegrationNetworkManager.getIntegrationConfig { [weak self] (model, message, status) in
            ...
            if isEntered == false {
                ...
                DispatchQueue.main.async {
                    let popView = TSAgreementPopView(title: String(format: "显示_积分规则".localized, TSAppConfig.share.localInfo.goldName), content: netModel.rule, doneTitle: "知道了")
                    popView.show()
                }
            }
        }
...
class IntegrationHomeTableController: UITableViewController {
    ...
    var showIAP: Bool {
        // 优先判断版本
        if TSAppConfig.share.environment.isAppStoreVersion == true {
            return true
        }
        return TSAppConfig.share.localInfo.showOnlyIAP
    }
    /// 积分充值显示开关 cell
    var showRechargeCell: Bool {
        ...
        return showIAP || TSAppConfig.share.localInfo.showIntegration
    }
    ///  积分提现显示开关 cell
    var showCashCell: Bool {
        return TSAppConfig.share.localInfo.showIntegrationRecharge && showIAP == false
    }
