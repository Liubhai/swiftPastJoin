        ...
        WalletNetworkManager.getConfig { [weak self] (status, message, model) in
            ...
            if isEntered == false {
                ...
                DispatchQueue.main.async {
                    let popView = TSAgreementPopView(title: "充值提现规则", content: netModel.rule, doneTitle: "知道了")
                    popView.show()
                }
            }
        }
...
class WalletHomeTableController: UITableViewController {
    ...
    var shouRechargeCell: Bool {
        return TSAppConfig.share.localInfo.showRecharge
    }
    /// 是否显示提现 cell
    var showCashCell: Bool {
        return TSAppConfig.share.localInfo.showCash
    }
    /// 是否显示积分充值 cell
    var showIntegrationRechargeCell: Bool {
        return showIAP || TSAppConfig.share.localInfo.showIntegrationRecharge
    }
    ...
    func setUI() {
        ...
        if TSUserInterfacePrinciples.share.isiphoneX() == true {
            ...
            headerView.backgroundColor = UIColor(hex: 0x59b6d7)
