...
class RankListDetailController: TSViewController {
    ...
    func setUI() {
        ...
        rankListView.rankNumberSpecialColors = [0: UIColor(hex: 0x59b6d7), 1: UIColor(hex: 0x59b6d7), 2: UIColor(hex: 0x59b6d7)]
        view = rankListView
    }
}
...
extension RankListDetailController: RankListDetailViewRefreshDelegate {
    ...
    func process(userInfos: [TSUserInfoModel]?) -> [Any]? {
        ...
        if rankType == .quoraExperts {
            datas = userInfos.map { RankListDetailCellModel(userInfo: $0, detailInfo: "问答点赞量：\($0.extra?.lastCheckinCount ?? 0)") }
        }
