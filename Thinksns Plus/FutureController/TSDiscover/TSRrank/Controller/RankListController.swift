...
class RankListController: UIViewController {
    ...
    func setUI() {
        title = "排行榜"
        // 1.标签滚动视图
        let titles = ["用户", "问答", "动态", "资讯"]
        labelCollection.leadingAndTralling = 32
        labelCollection.titles = titles
        labelCollection.shouldShowBlueLine = true
        // 2.列表视图
        // 用户排行榜的榜单类型
        let userRankTypes: [RankListManager.RankType] = [.fans, .wealth, .income, .attendance, .communityExperts, .quoraExperts]
        // 问答排行榜的榜单类型
        ...
        let allRankTypes = [userRankTypes.map { $0.rawValue }, quoraRankTypes.map { $0.rawValue }, feedRankTypes.map { $0.rawValue }, newsRankTypes.map { $0.rawValue }]
