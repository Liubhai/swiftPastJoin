...
class TSNewsDetailHeaderViewController: UIViewController, TSDetailRewardListViewDelegate, TSChoosePriceVCDelegate {
    ...
    func load(_ data: NewsDetailModel?) {
        ...
        self.categoryLabel.frame = CGRect(x: 10, y: self.titleLabel.frame.maxY + 10, width: self.categoryLabel.frame.width + 5, height: self.categoryLabel.frame.height)
        var from = ""
        if data.from == "原创" { // 垃圾后台要求这样判断的
            from = String(format: "%@", data.author)
        } else {
            from = String(format: "来自 %@", data.from)
        }
        self.fromLabel.text = from
        self.fromLabel.sizeToFit()
        self.fromLabel.frame = CGRect(x: self.categoryLabel.frame.maxX + 5, y: self.titleLabel.frame.maxY + 10, width: self.fromLabel.frame.width, height: self.categoryLabel.frame.height)

        let timeString = TSDate().dateString(.detail, nsDate: data.createdDate as NSDate)
        labelForSubInfo.text = "发布于\(timeString)\n" + TSAppConfig.share.pageViewsString(number: data.hits!) +  "显示_人浏览".localized
    ...
    override func viewDidLayoutSubviews() {
        ...
        labelForSubInfo.frame = CGRect(x: self.view.frame.width - 10 - labelForSubInfo.frame.width, y: self.markdownView.frame.maxY + 23, width: labelForSubInfo.frame.width, height: labelForSubInfo.frame.height)
        if TSAppConfig.share.localInfo.isOpenReward == true {
